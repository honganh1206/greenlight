...
func (app *application) routes() []route {
	return []route{
		...
		newRoute(http.MethodPost, MovieV1, app.requirePermission("movies:write", app.createMovieHandler)),
		newRoute(http.MethodGet, MovieV1, app.requirePermission("movies:read", app.listMovieHandler)),
		newRoute(http.MethodGet, MovieV1+"/([0-9]+)", app.requirePermission("movies:read", app.showMovieHandler)),
		newRoute(http.MethodPatch, MovieV1+"/([0-9]+)", app.requirePermission("movies:write", app.updateMovieHandler)),
		newRoute(http.MethodDelete, MovieV1+"/([0-9]+)", app.requirePermission("movies:write", app.deleteMovieHandler)),
